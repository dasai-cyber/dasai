            ...
            {COMPANY_DATA.services.map((service, idx) => {
              ...
              return (
                <div
                  key={service.slug}
                  id={service.slug}
                  className="p-8 sm:p-12 rounded-3xl bg-slate-50 border border-slate-200 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center"
                >
                  ...
                  <div className={`space-y-6 ${isEven ? "lg:col-span-7 lg:order-2" : "lg:col-span-7"}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <h3 className="text-2xl sm:text-3xl font-black text-[#0F2C59] mt-1">
                          {service.title}
                        </h3>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 pt-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {service.features.map((feat, fIdx) => (
                          <div
                            key={fIdx}
                            className="flex items-start gap-2 text-xs sm:text-sm text-slate-700 bg-white p-2.5 rounded-xl border border-slate-200"
                          >
                            <Check className="w-4 h-4 text-[#00C29A] shrink-0 mt-0.5" />
                            <span>{feat}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className={`space-y-4 ${isEven ? "lg:col-span-5 lg:order-1" : "lg:col-span-5"}`}>
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
                      <h4 className="text-xs font-black uppercase tracking-wider text-[#0F2C59] pb-2 border-b border-slate-100">
                      ...
                      <h4 className="text-xs font-black uppercase tracking-wider text-[#0F2C59] pt-2 pb-1 border-b border-slate-100">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {service.vehicles.map((veh, vIdx) => (
                          <span
                            key={vIdx}
                            className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-900 font-semibold text-[11px] border border-blue-100"
                          >
                            {veh}
                          </span>
                        ))}
